...
- 2026-05-22: **定制表编辑模块 (v1.0.0)**：新增跨服务器、跨数据库的通用表编辑功能。采用方案C（混合式）：配置驱动 + 可覆盖自定义模板。新增文件：`config/custom_tables.toml`（服务器和表配置）、`modules/custom_table/`（config_loader + table_service）、`routes/custom_table_routes.py`（蓝图，9个路由）、`templates/custom_table/`（index.html + editor.html）。入口：首页卡片 + 导航栏。三模式编辑器（参考调车模块 config.html）：可视化编辑（左侧分组导航 + 右侧记录卡片表单）、表格编辑（分组折叠 + 双击内联编辑）、源数据（JSON 批量编辑）。支持 dirty tracking 批量保存。当前配置 10.68.2.17 的 fy_bl_save_task_config 表（219条记录，6字段）。详细设计见 `plans/custom_table_editor_design.md`。