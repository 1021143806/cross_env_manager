...
- 🔄 **RCS模板同步**: 大模板级别四表同步（agv_model / agv_robot / agv_robot_ext / task_template）
...
- 🛡️ **解死锁冷却期**: 解死锁取消后自动进入冷却期，避免 in/out 方向互相阻塞形成死循环
- ✅ **空车下发 Idle 检查**: 下发前实时查询 ICS 确认设备空闲，避免下发到 InTask 设备
- 📊 **折线图统计修复**: status=10 完成时也触发 daily_stats 统计，解决折线图为 0 的问题
- 🧹 **取消失败清理**: 解死锁取消失败时也清理本地 JSON，避免重复尝试取消
- 📋 **任务下发模块**: 楼栋分组下拉框 + 今日使用统计 + 后端代理查询 + 设备号查询 + 一键复制 + 指定设备下发（agv_robot_ext 映射 + 搜索式下拉框）
- ⏏️ **辊筒任务支持**: 非跨环境任务独立分组下发，固定点位配置，已下发缓存管理+容量管控，orderId 前缀 `RLLR_` 自动路由查询
- 🔌 **交接点配置**: 跨环境交接点配对管理（join_qr_node_info），模板配置检查
- 🔗 **多数据库连接池**: 生产/测试库连接池分离，支持生产库只读查询
