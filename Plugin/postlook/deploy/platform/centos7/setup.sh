#!/bin/bash
# ============================================================
# postlook · CentOS 7 专属 Python 安装
# 仅在 detect.sh 判定系统 Python < 3.9 时被 python.sh 调用
# ============================================================
#
# 安装策略（按优先级）:
...
CENTOS7_RPM_DIR="$CENTOS7_PLATFORM_DIR/rpms"

log_info "CentOS 7 Python 安装策略..."

# ---- 策略 1：预编译包 ----
PYTHON_TGZ="$CENTOS7_RPM_DIR/python39_build.tar.gz"
...
# ---- 策略 2：源码编译 ----
PYTHON_SRC="$CENTOS7_RPM_DIR/Python-3.9.20.tar.xz"
...
for rpm_pkg in openssl-devel bzip2-devel libffi-devel; do
    if ! rpm -qa | grep -q "^${rpm_pkg}-"; then
        local rpm_file=$(ls "$CENTOS7_RPM_DIR/${rpm_pkg}"*.rpm 2>/dev/null | head -1)
        if [ -f "$rpm_file" ]; then
            log_info "安装 $rpm_pkg..."
