...
document.addEventListener('DOMContentLoaded', function () {
    ...
    window.showTopoDetail = function (node) {
        try {
            ...
            if (logFile && logDir) html += '<button class="file-actions" style="padding:4px 12px" onclick="window.location.href=\'/api/download?path=' + encodeURIComponent(logDir + '/' + logFile) + '\'">⬇ 下载</button>';
                        ...
                        for (var i = 0; i < data.directories.length; i++) {
                            ...
                            if (d.path === logDir || d.path.indexOf(logDir + '/') === 0) {
                                for (var j = 0; j < Math.min(d.files.length, 10); j++) {
                                    var f = d.files[j];
                                    items += '<div class="file-row"><span class="file-name" title="' + f.path + '">' + f.name + '</span><span class="file-size">' + formatBytes(f.size) + '</span><div class="file-actions"><button onclick="window.location.href=\'/api/download?path=' + encodeURIComponent(f.path) + '\'">⬇</button><button onclick="viewTopoLog(\'' + encodeURIComponent(f.path) + '\')">👁</button></div></div>';
                                }
                                break;
                            }
                        }
    ...
    window.showKgPanel = function (node) {
        ...
        if (triples.length === 0) {
            ...
            downloadPaths.forEach(function (dp) {
                ...
                html += '<button class="kg-dl-btn" onclick="window.location.href=\'/api/download?path=' + encodeURIComponent(dp.path) + '\'" title="下载">⬇</button>';
