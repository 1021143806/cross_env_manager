...
document.addEventListener('DOMContentLoaded', function () {
    ...
    window.showTopoDetail = function (node) {
        try {
            ...
            if (logFile && logDir) html += '<a class="file-actions" style="padding:4px 12px;display:inline-block;text-decoration:none;cursor:pointer" href="/api/download?path=' + encodeURIComponent(logDir + '/' + logFile) + '" target="_blank">⬇ 下载</a>';
                        ...
                        for (var i = 0; i < data.directories.length; i++) {
                            ...
                            if (d.path === logDir || d.path.indexOf(logDir + '/') === 0) {
                                for (var j = 0; j < Math.min(d.files.length, 10); j++) {
                                    var f = d.files[j];
                                    items += '<div class="file-row"><span class="file-name" title="' + f.path + '">' + f.name + '</span><span class="file-size">' + formatBytes(f.size) + '</span><div class="file-actions"><a href="/api/download?path=' + encodeURIComponent(f.path) + '" target="_blank" style="cursor:pointer">⬇</a><button onclick="viewTopoLog(\'' + encodeURIComponent(f.path) + '\')">👁</button></div></div>';
                                }
                                break;
                            }
                        }
    ...
    window.showKgPanel = function (node) {
        ...
        if (triples.length === 0) {
            ...
            downloadPaths.forEach(function (dp) {
                ...
                html += '<a class="kg-dl-btn" href="/api/download?path=' + encodeURIComponent(dp.path) + '" target="_blank" title="下载" style="cursor:pointer">⬇</a>';
