...
document.addEventListener('DOMContentLoaded', function () {
    ...
    window.showTopoDetail = function (node) {
        try {
            ...
            if (logFile && logDir) html += '<button class="file-actions" style="padding:4px 12px" onclick="var a=document.createElement(\'a\');a.href=\'/api/download?path=' + encodeURIComponent(logDir + '/' + logFile) + '\';document.body.appendChild(a);a.click();document.body.removeChild(a)">⬇ 下载</button>';
                        ...
                        for (var i = 0; i < data.directories.length; i++) {
                            ...
                            if (d.path === logDir || d.path.indexOf(logDir + '/') === 0) {
                                for (var j = 0; j < Math.min(d.files.length, 10); j++) {
                                    var f = d.files[j];
                                    items += '<div class="file-row"><span class="file-name" title="' + f.path + '">' + f.name + '</span><span class="file-size">' + formatBytes(f.size) + '</span><div class="file-actions"><button onclick="var a=document.createElement(\'a\');a.href=\'/api/download?path=' + encodeURIComponent(f.path) + '\';document.body.appendChild(a);a.click();document.body.removeChild(a)">⬇</button><button onclick="viewTopoLog(\'' + encodeURIComponent(f.path) + '\')">👁</button></div></div>';
                                }
                                break;
                            }
                        }
    ...
    window.showKgPanel = function (node) {
        ...
        if (triples.length === 0) {
            ...
            // 补充：从关联的 logfile 子节点获取路径（produces + contains）
            cy.edges().forEach(function (e) {
                var rel = e.data('relation') || '';
                if (rel !== 'produces' && rel !== 'contains') return;
            ...
            downloadPaths.forEach(function (dp) {
                ...
                html += '<button class="kg-dl-btn" onclick="var a=document.createElement(\'a\');a.href=\'/api/download?path=' + encodeURIComponent(dp.path) + '\';document.body.appendChild(a);a.click();document.body.removeChild(a)" title="下载">⬇</button>';
