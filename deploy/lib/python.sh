...
ensure_python() {
    ...
    case "$OS_ID" in
        centos)
            ...
            ;;
        openEuler)
            # openEuler/IRAYPLEOS 开放版策略
            if [ -f "$DEPLOY_DIR/platform/openEuler/setup.sh" ]; then
                source "$DEPLOY_DIR/platform/openEuler/setup.sh"
            else
...
create_venv() {
    ...
    # conda Python 环境：让 venv 继承系统站点包
